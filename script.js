...
// دالة تنظيف النص من الالتصاقات الناتجة عن السحب
function fixStuckWords(text) {
    ...
    formatted = formatted.replace(/([اأإآؤئءيىوَةتثجحخدذرزسشصضطظعغفقكلمنه])قَالَ/g, '$1 قَالَ');
    formatted = formatted.replace(/([اأإآؤئءيىوَةتثجحخدذرزسشصضطظعغفقكلمنه])قَدْ/g, '$1 قَدْ');

    // الحل الجذري لمشكلة حذف الآيات: تحديد النطاق بـ 60 حرف كحد أقصى لحذف أرقام الصفحات فقط
...
function formatArabicHTML(text) {
    ...
    // ترتيب صارم للرواة لفصل الفقرات بدون ترك حروف معلقة
...
function openModal(id) {
    ...
    // إرجاع التمرير للأعلى (مهمة جداً لحل مشكلة السكرول)
    setTimeout(() => {
